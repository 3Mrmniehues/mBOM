(function () {
  ...
  if (copyBomBtn) {
    copyBomBtn.addEventListener("click", () => {
      // Tree view has a leading toggle column and a trailing Actions column;
      // Flat view is all data columns.
      // Tree view: skip the leading select + toggle columns and the trailing
      // Actions column. Flat view is all data columns.
      const opts = currentView === "tree" ? { skipLeading: 2, skipTrailing: 1 } : {};
      copyGridToClipboard(document.getElementById("bomTable"), opts);
    });
  }
  if (copyOrdersBtn) {
    copyOrdersBtn.addEventListener("click", () => {
      copyGridToClipboard(document.getElementById("ordersTable"), {
        skipTrailing: 1, // Actions column
        skipRowClass: ["order-parts-row"], // don't copy expanded parts sub-tables
      });
    });
  }
  ...
  function renderTreeView() {
    ...
    const colDefs = [
      { key: "__select__", width: SELECT_COL_WIDTH },
      { key: "__toggle__", width: toggleColWidth },
    ];
    ...
    headThs.forEach((th, idx) => {
      if (idx <= 1) return; // select + toggle columns aren't resizable
  ...
  function htmlTableSection(title, headers, rowsHtml, note) {
    if (!rowsHtml) {
      return '<section><h2>' + xmlEscape(title) + '</h2><p class="empty">Nothing to show.</p></section>';
    }
    const thead = "<tr>" + headers.map((h) => "<th>" + xmlEscape(h.label) +
      "</th>").join("") + "</tr>";
    return (
      '<section><h2>' + xmlEscape(title) + "</h2>" +
      (note ? '<p class="section-note">' + xmlEscape(note) + "</p>" : "") +
      '<div class="table-scroll"><table><thead>' + thead + "</thead><tbody>" + rowsHtml + "</tbody></table></div></section>"
    );
  }

  function tsvCell(value, cls, style) {
    return '<td' + (cls ? ' class="' + cls + '"' : "") + (style ? ' style="' + style + '"' : "") +
      ">" + xmlEscape(value) + "</td>";
  }

  function buildProjectHtmlDoc() {
    ...
    const flat = [];
    flattenTreeForExport(tree, 0, flat);
    const treeRowsHtml = flat.map(({ node, depth }) => {
      const info = resolveOrderInfo(node, parentMap);
      const tds = cols.map((c) => {
        let value;
        if (c.key === "itemNo") value = itemNumbers.get(node.guid) || "";
        else if (c.key === "po") value = info.po;
        else if (c.key === "rfx") value = info.rfx;
        else if (c.key === "status") value = info.status;
        else if (c.type === "checkbox") value = getExportFieldValue(node, c) === "X" ? "✓" : "";
        else value = getExportFieldValue(node, c);
        const cls = c.key === "itemNo" || c.type === "number" ? "num" : c.type === "checkbox" ? "ctr" : "";
        const style = c.key === "description" ? "padding-left:" + (depth * 16 + 8) + "px" : "";
        return tsvCell(value, cls, style);
      });
      return '<tr class="lvl' + Math.min(depth, 5) + (node.assy ? " assy" : "") + '">' + tds.join("") + "</tr>";
    }).join("");

    // --- BOM Flat (aggregated by part number) ---
    const flatCols = getExportAggregateColumns(FLAT_EXCLUDED_KEYS);
    const flatAgg = aggregateByPartNumber(collectFlatNodes(tree)).sort((a, b) => a.partNumber.localeCompare(b.partNumber));
    const flatRowsHtml = flatAgg.map((r) => {
      const tds = flatCols.map((c) => {
        let value = getExportFieldValue(r, c);
        if (c.type === "checkbox") value = value === "X" ? "✓" : "";
        const cls = c.type === "number" ? "num" : c.type === "checkbox" ? "ctr" : "";
        return tsvCell(value, cls, "");
      });
    ...
    const orderRowsHtml = orders.map((o) => {
      const tds = ORDER_COLUMNS.map((c) => tsvCell(o[c.key] == null ? "" : o[c.key], "", ""));
      return "<tr>" + tds.join("") + "</tr>";
    }).join("");

    // --- Project details ---
    const detailPairs = [
      ["WBS", project.wbs], ["EWR", project.ewr], ["Project Name", project.name],
      ["Status", project.status], ["Date Created", project.dateCreated], ["Exported", generated],
    ];
    const detailRowsHtml = detailPairs
      .map((p) => "<tr><th>" + xmlEscape(p[0]) + "</th>" + tsvCell(p[1] == null ? "" : p[1], "", "") + "</tr>")
      .join("");

    const levelCss = LEVEL_FILL_COLORS
      .map((c, i) => "tr.lvl" + i + " td{background:#" + c.slice(2) + ";}")
      .join("");

    const titleText = (project.wbs ? project.wbs + " — " : "") + (project.name || "Project") + " — BOM Snapshot";

    return (
      "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">" +
      '<meta name="viewport" content="width=device-width, initial-scale=1">' +
      "<title>" + xmlEscape(titleText) + "</title><style>" +
      "*{box-sizing:border-box;}" +
      "body{margin:0;padding:24px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#1f2430;background:#f5f6f8;}" +
      ".wrap{max-width:1500px;margin:0 auto;}" +
      "header{margin-bottom:20px;}header h1{margin:0 0 4px;font-size:1.5rem;}" +
      "header .meta{color:#6b7280;font-size:0.9rem;}" +
      "section{background:#fff;border:1px solid #dfe3e8;border-radius:8px;padding:16px;margin-bottom:20px;}" +
      "section h2{margin:0 0 12px;font-size:1.1rem;}" +
      ".section-note{margin:0 0 10px;color:#6b7280;font-size:0.8rem;}" +
      ".table-scroll{overflow-x:auto;}" +
      "table{border-collapse:collapse;width:100%;font-size:0.82rem;}" +
      "th,td{border:1px solid #e6e9ee;padding:5px 8px;text-align:left;white-space:nowrap;}" +
      "thead th{background:#eef2f7;font-weight:600;position:sticky;top:0;}" +
      "td.num{text-align:right;font-variant-numeric:tabular-nums;}td.ctr{text-align:center;}" +
      "tr.assy td{font-weight:600;}" +
      ".details-table{max-width:520px;}.details-table th{background:#f5f6f8;width:150px;}" +
      ".empty{color:#6b7280;font-size:0.85rem;}" +
      ".footer-note{color:#6b7280;font-size:0.8rem;text-align:center;margin-top:8px;}" +
      levelCss +
      "</style></head><body><div class=\"wrap\">" +
      "<header><h1>" + xmlEscape(project.name || "Project") + "</h1>" +
      '<div class="meta">WBS ' + xmlEscape(project.wbs || "—") + " · EWR " + xmlEscape(project.ewr || "—") +
      " · Status " + xmlEscape(project.status || "—") + " · Exported " + xmlEscape(generated) + "</div></header>" +
      '<section><h2>Project Details</h2><table class="details-table"><tbody>' + detailRowsHtml + "</tbody></table></section>" +
      htmlTableSection("Bill of Materials — Tree", cols, treeRowsHtml,
        "Full hierarchy. Rows are shaded and the Description indented by depth; Item No shows the position (e.g. 1.7.1.1).") +
      htmlTableSection("Bill of Materials — Flat (by 3M Part Number)", flatCols, flatRowsHtml,
        "Aggregated across the tree; Total Qty rolls up sub-item quantities.") +
      htmlTableSection("Orders", ORDER_COLUMNS, orderRowsHtml, null) +
      '<p class="footer-note">Read-only snapshot generated by the mBOM app. No live data or database connection.</p>' +
      "</div></body></html>"
    );
  }

  function exportProjectHtml() {
    const html = buildProjectHtmlDoc();
    const baseName = (project.wbs || project.name || "project").replace(/[^a-z0-9\-_.]+/gi, "_");
    downloadFile(html, baseName + "-snapshot.html", "text/html;charset=utf-8");
    showToast("Exported project snapshot to HTML");
  }
